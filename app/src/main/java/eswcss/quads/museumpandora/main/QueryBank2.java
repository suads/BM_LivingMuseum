...
//			"?mat skos:prefLabel ?material .}" +
    ...
    public static String getObjectByLocation = "PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>" +
            ...
            " OPTIONAL { ?obj crm:P102_has_title ?titleO . ?titleO rdfs:label ?title .} " +
            " OPTIONAL { Filter(!bound(?title)) ?obj crm:P128_caries ?x . ?x rdfs:label ?title . } " +
            " OPTIONAL { Filter(!bound(?title)) ?ep crm:P70_documents ?obj . ?ep dc:title ?title  . } " +
            ...
            "OPTIONAL { ?ep purl:medium_synopsis ?mediumSynopsis . }" +
            "OPTIONAL { ?ep purl:long_synopsis ?longSynopsis . }" +
            "OPTIONAL { ?page foaf:primaryTopic ?ep ." +
            "?page dct:created ?date . } " +
//   "OPTIONAL {?obj crm:P45_consists_of ?mat ." +
//   "?mat skos:prefLabel ?material .}" +
            "OPTIONAL{?obj crm:P46i_forms_part_of ?collec ." +
            ...
            "} " + //GROUPBY (?obj ?ep ?title ?image ?desc ?shortSynopsis ?mediumSynopsis ?longSynopsis ?location ?collection ?culture ?foundat ?objectType)"
            " LIMIT " + limit ;
    ...
    public static String getStartObject = "PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>" +
            ...
            " OPTIONAL {{ ?ep crm:P70_documents ?obj . " +
            "?ep dc:title ?title  . } " +
            " UNION { ?obj crm:P128_caries ?x . ?x rdfs:label ?title . } " +
            " UNION { ?obj crm:P102_has_title ?titl . ?titl rdfs:label ?title . }" +
            "}  " +
